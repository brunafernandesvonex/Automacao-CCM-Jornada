...
export function validarInformacoesDaJornadaReceberMensagemFluxoBasico() {
    cy.get(elemento.pesquisar).should('be.visible').click({ force: true }).type('[NÃO CANCELAR] Teste automatizado 01').wait(1000);
    cy.get(elemento.verJornada).should('be.visible').should('not.be.disabled').click();
    cy.wait(3000);
    cy.get(elemento.nomeDaJornada).should('have.value', '[NÃO CANCELAR] Teste automatizado 01');
    ...
    cy.get(elemento.pesquisar).should('be.visible').click({force: true}).type('[NÃO CANCELAR] Teste automatizado 01');
}
...
export function validarInformacoesDaJornadaReceberMensagemFluxoEscolherOpcao() {
    cy.get(elemento.pesquisar).should('be.visible').click({ force: true }).type('[NÃO CANCELAR] Teste automatizado 02').wait(1000);
    cy.get(elemento.verJornada).should('be.visible').should('not.be.disabled').click();
    cy.wait(3000);
    ...
    cy.get(elemento.fluxoDaJonada, { timeout: 10000 }).should('be.visible');
    cy.get(elemento.botaoVoltarMenu).should('be.visible').click();
    cy.get(elemento.pesquisar).should('be.visible').click({force: true}).type('[NÃO CANCELAR] Teste automatizado 02');
}